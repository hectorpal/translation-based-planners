...
int main(int argc, const char **argv) {
	...
	ofstream stats( "execution.stats" );
	stats << "Preprocessing time=" << (total_ms / 1000.0) - (search_ms / 1000.0) << endl;	
	stats << "Search time=" << (search_ms / 1000.0) << endl;
	stats << "Total time=" << (total_ms / 1000.0) << endl;

        if(engine->found_solution())
     	   stats << "Plan Cost="<<plan_cost<<endl;	    
