...
void LandmarksGraph::generate() {
  struct tms order1_start, order1_end;
  struct tms order2_start, order2_end;
  ...
  if(reasonable_orders) {
    ...
    times(&order1_end);
    int order1_ms = (order1_end.tms_utime - 
		     order1_start.tms_utime) * 10;
    cout << "Approx reasonable order time: " << order1_ms / 1000.0 
	 << " seconds" << endl;
    ...
    times(&order2_end);
    int order2_ms = (order2_end.tms_utime - 
		     order2_start.tms_utime) * 10;
    cout << "Approx obedient reasonable order time: " << order2_ms / 1000.0 
	 << " seconds" << endl;
  }
